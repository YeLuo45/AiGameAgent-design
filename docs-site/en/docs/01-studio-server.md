...
| POST | `/api/projects` | Create new project (default title: "Default Project") |
...
| LLM returns non-JSON | Try `parseMeetingTranscriptJson` → fall back to `parseMeetingTranscriptLoose` (regex on `Secretary:...`) |

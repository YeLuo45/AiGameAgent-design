...
If all three fail, a regex on `^(Secretary|Producer|Technical Director|Creative Director)\s*[:：]\s*(.+)$` parses line-by-line. This works for `Q: ... A: ...` patterns even when the model is too small to follow JSON instructions.
...
> **Scenario: Cursor streams visible events via the proxy**
> - **WHEN** a user points an OpenAI-compatible client (e.g. Cursor) `baseURL` at `http://127.0.0.1:<studio-port>/v1`
> - **THEN** the system SHALL record and dispatch `llm.chunk` and `llm.message_done` events, for the Studio Web UI to display in real time

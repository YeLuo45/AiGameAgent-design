...
> **The cardinal rule**: "Do not invent `wx` method signatures" — only use APIs documented by WeChat or already typed in the repo. The `wx.d.ts` shipped with the DevTools is the source of truth.
...
> Same cardinal rule: "Do not invent `tt` APIs" — only what the docs or repo typings say.

## Shared logic

The `STUDIO.md` and `docs/COLLABORATIVE-DESIGN-PRINCIPLE.md` are explicit:

> **Shared logic**: `packages/shared/` (or `src/shared/`); platform code MUST NOT reference WeChat/Douyin global objects directly from `src/web/`.
...
1. Boss starts a meeting with topic "make a snake variant"

...
- **Approve** → enqueue a "summary + next steps" task to the producer for that project
- **Reject** → enqueue a "list issues and re-do" task
- **Redo** → enqueue a "confirm goals and re-execute" task

Each one is a real `POST /api/dept/workorder/action` — the policy decides the agent's provider.

## "Meeting" drawer — the kickoff flow

1. Boss types a `topic` (or "(topic to be added)")
2. Toggles `Skip LLM` for a rule-based meeting
3. Clicks **Start meeting**
4. UI shows a streaming transcript: each line is `Speaker: text`
5. Three speakers take turns: Producer, Technical Director, Creative Director
6. Boss sees a **charter form** (goal, milestones, nodes) they can edit
7. **Archive** freezes a version
8. **Save draft** persists without bumping version
9. When the draft diverges from the archive, the meeting shows `Pending drift: ...`
...
The "auto-save" magic: when the reducer detects that an agent's `streamDraft` ends with `</html>`, it POSTs to `/api/preview/save` automatically. The secretary HUD narrates "Secretary: complete HTML detected, auto-saved to the monitor".
